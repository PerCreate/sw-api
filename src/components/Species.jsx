    ...
    return (
      <React.Fragment>
        ...
        <div className="lists">
          ...
          <div className={`list-data ships ${rocketState}`}>{people.length !== 0 ? people.map((person) => <Link onClick={(e)=> e.preventDefault()} className='to-films-link'><div className='list-note' key={person.name}>{person.name}</div></Link>) : 
